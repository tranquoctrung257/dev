...
def login(author):
    head_login = {
        ...
        "T": "VFZSamQwMUVRVFZOZWtreVQwRTlQUT09"
    }
    try:
        account = requests.get("https://sv5.golike.net/api/tiktok-account",headers=head_login,impersonate="safari_ios").json()
...
if (select == 2):
    ...
    t = "VFZSak1VNVVaM2xQUkUweVRrRTlQUT09"
...
while True:
    ...
    while True:
        try:
            getjob = requests.get("https://sv5.golike.net/api/advertising/publishers/tiktok/jobs?account_id="+str(listid[stt])+"&data=null",headers=head_login,impersonate="safari_ios").json()
    ...
    os.system(f'adb -s AMR00365548 shell am start -a android.intent.action.VIEW -d {link}')
    # os.system('start '+link)
    for i in range(5,0,-1):
        if (type_acction == "follow"):
            print(syan+'VUI LÒNG THỰC HIỆN FOLLOW TRONG '+trang+str(i)+syan+' GIÂY                                  ',end= "\r")
            time.sleep(3)
            os.system(" adb -s AMR00365548 shell input tap 161 561")
            break
            ...
            os.system(" adb -s AMR00365548 shell input tap 639 895")
            break
        time.sleep(1)
    ...
    getxu = requests.post('https://gateway.golike.net/api/advertising/publishers/tiktok/complete-jobs',data=dt,headers=hd,impersonate="safari_ios").json()
    if (getxu['status'] != 200):
        getxu1 = requests.post('https://sv5.golike.net/api/advertising/publishers/tiktok/complete-jobs',data=dt,headers=hd,impersonate="safari_ios").json()
    ...
    if (success == 1):
        ...
        skipjob = requests.post('https://sv5.golike.net/api/advertising/publishers/tiktok/skip-jobs',impersonate="safari_ios",data=dt1,headers=hd).json()
