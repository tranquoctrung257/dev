...
while True:
    ...
    os.system(f'adb -s BH90152A99 shell am start -a android.intent.action.VIEW -d {link}')
    # os.system('start '+link)
    for i in range(5,0,-1):
        if (type_acction == "follow"):
            print(syan+'VUI LÒNG THỰC HIỆN FOLLOW TRONG '+trang+str(i)+syan+' GIÂY                                  ',end= "\r")
            time.sleep(3)
            os.system(" adb -s BH90152A99 shell input tap 320 743")
            break
        elif (type_acction == "like"):
            print(syan+'VUI lÒNG THỰC HIỆN THẢ TIM TRONG '+trang+str(i)+syan+' giây                                 ',end= "\r")
            time.sleep(3)
            # os.system(" adb -s BH90152A99 shell input tap 999 817")
            os.system(" adb -s BH90152A99 shell input tap 996 917")
            break
        time.sleep(1)
